...
int main (int argc, char *argv[]){
    ...
    size_t T = 116;
    vector<vector<int>> priorityGroupMatrix(T, vector<int>(n, 0));
    string priorityGroupMatrixFilePath = "./cluster_matrices_GrENMF/cluster_results_GEANT_GrENMF.csv";
    readCSV(priorityGroupMatrixFilePath, priorityGroupMatrix, T, n);
    ...
    OutputIpLookupTable(ipLookupTable, "IpLookupTable_priority.csv");
    ...
    for(size_t i = 0; i < n; i++){
        for(size_t j = 0; j < n; j++){
            ...
            dest.SetTos((priorityGroupMatrix[t][i] == priorityGroupMatrix[t][j]) ? 4 : 12); // TODO: change the priority! 4=high priority, 12 = low priority
    ...
    p2pHelper.EnablePcapAll("real2_priority_"+to_string(t));
