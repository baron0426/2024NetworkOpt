...
def parse_pcap(node_num, pcap_files, ip_lookup_dict, output_file):
    with open(output_file, 'w') as file:
        file.write('time,src_node,dst_node\n')
        for pcap_file in pcap_files:
            ...
            for packet in packets:
                ...
                file.write("{},{},{}\n".format(packet.time, src_node, dst_node))
...
def sort_result(csv_file):
    with open(csv_file, mode='r', newline='') as file:
        ...
        header = next(reader)
        rows = list(reader)
    sorted_rows = sorted(rows, key=lambda x: (int(x[1]), int(x[2]), float(x[0])))  # Assuming Time column is numeric
